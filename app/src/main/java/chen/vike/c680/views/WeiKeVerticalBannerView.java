...
import chen.vike.c680.adapter.WeiKeBaseBannerAdapter;

/**
 * 轮播view
 */
public class WeiKeVerticalBannerView extends LinearLayout implements WeiKeBaseBannerAdapter.OnDataChangedListener {

    private float mBannerHeight = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,1,getResources().getDisplayMetrics());
    private int mGap = 4000;
    private int mAnimDuration = 1000;

    private WeiKeBaseBannerAdapter mAdapter;
    ...
    public WeiKeVerticalBannerView(Context context) {
        this(context,null);
    }

    public WeiKeVerticalBannerView(Context context, AttributeSet attrs) {
        this(context, attrs,0);
    }

    public WeiKeVerticalBannerView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.WeiKeVerticalBannerView);
        mGap = array.getInteger(R.styleable.WeiKeVerticalBannerView_gap,mGap);
        mAnimDuration = array.getInteger(R.styleable.WeiKeVerticalBannerView_animDuration,mAnimDuration);
     ...
     * 设置banner的数据
     * */
    public void setAdapter(WeiKeBaseBannerAdapter adapter){
        if(adapter == null){
