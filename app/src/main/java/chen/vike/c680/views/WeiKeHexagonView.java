...
import chen.vike.c680.tools.LassoUtils;
...
public class WeiKeHexagonView extends android.support.v7.widget.AppCompatImageView {
    ...
    private Paint paint, paint1,paint2;// 画笔,文字
    private String text = "";// 文字
    private int textSize = -1;// 字体大小
    private int textColor;// 字体颜色
    private Drawable src;// 图片
    private int backColor,linback;// 背景色
   private int radius = 4;//圆角度
    // 监听
    private OnHexagonViewClickListener listener;
    // 判断点是否在多边形内
    LassoUtils lasso,lasso1;// 核心判断类
    List<PointF> list,list1;
    ShapeDrawable mDrawable;
    Animation scaleAnimation,endAnimation;

    public WeiKeHexagonView(Context context) {
        super(context);
    }
     ...
     * 构造函数
     */
    public WeiKeHexagonView(Context context, AttributeSet attrs) {
        super(context, attrs);
        // Log.v(TAG, "HexagonView()-2");
        // ------自定义属性
        // 将属性连接起来
        TypedArray typeA = context.obtainStyledAttributes(attrs,
                R.styleable.WeiKeHexagonView);
        // 获取控件属性值
        text = typeA.getString(R.styleable.WeiKeHexagonView_text);
        textSize = typeA.getDimensionPixelSize(
                R.styleable.WeiKeHexagonView_textSize, -1);
        textColor = typeA.getColor(R.styleable.WeiKeHexagonView_textColor,
                Color.BLUE);
        src = typeA.getDrawable(R.styleable.WeiKeHexagonView_src);
        backColor = typeA.getColor(R.styleable.WeiKeHexagonView_backColor,
                Color.BLACK);
        linback = typeA.getColor(R.styleable.WeiKeHexagonView_lineback,Color.GRAY);
         radius = typeA.getInteger(R.styleable.WeiKeHexagonView_radius,4);
        // 回收资源，一定要调用
        typeA.recycle();
        //-------
        //缩放动画
...
//        path.lineTo(vWidth - a, c);
        float x = vWidth / 2F;
        float y = vHeight / 2F;
        float temp = vLenght *(float)Math.sqrt(3) / 2;

        float stui = radius;//圆角度
        path.moveTo(x + stui,y - vLenght + stui);
        path.lineTo(x + temp - stui,y - vLenght /2 - stui);

        path.moveTo(x + temp,y - vLenght / 2 + stui);
        path.lineTo(x + temp,y + vLenght /2 - stui);

        path.moveTo(x + temp - stui,y + vLenght /2+stui);
        path.lineTo(x + stui,y + vLenght -stui);
        ...
        path.moveTo(x - stui,y - vLenght + stui);
        path.quadTo(x,y-vLenght,x + stui,y - vLenght + stui);

        path.moveTo(x + temp - stui,y - vLenght /2 - stui);
        path.quadTo(x + temp,y - vLenght /2,x + temp,y - vLenght / 2 + stui);

        path.moveTo(x + temp,y + vLenght /2 - stui);
        path.quadTo(x + temp,y + vLenght /2,x + temp - stui,y + vLenght /2+stui);

        path.moveTo(x + stui,y + vLenght -stui);
        path.quadTo(x,x + vLenght,x - stui,y +vLenght -stui);
      ...
      //   shadeShow(x,y,canvas);
        ...
        //六边形顶点位置
        if (list1 == null) {
            list1 = new ArrayList<PointF>();
        } else {
            list1.clear();
        }
        PointF pf = new PointF();
        pf.set(vWidth / 2F,vHeight / 2F - vLenght);
        list.add(pf);
        PointF pf1 = new PointF();
        pf1.set(x + vLenght * (float)Math.sqrt(3) / 2,y - vLenght / 2);
        list.add(pf1);
        PointF pf2 = new PointF();
        pf2.set(x + vLenght * (float)Math.sqrt(3) / 2,y + vLenght / 2);
        list.add(pf2);
        PointF pf3 = new PointF();
        pf3.set(vWidth / 2F,vHeight / 2F + vLenght);
        list.add(pf3);
        PointF pf4 = new PointF();
        pf4.set(x - vLenght * (float)Math.sqrt(3) / 2,y + vLenght / 2);
        list.add(pf4);
        PointF pf5 = new PointF();
        pf5.set(x - vLenght * (float)Math.sqrt(3) / 2,y - vLenght / 2);
        list.add(pf5);
        ...
        lasso1 = LassoUtils.getInstance();
        lasso1.setLassoList(list1);
     ...
     * @param canvas
     */
    private void shadeShow(float x,float y,Canvas canvas){
        Path path = new Path();
        if (paint2 == null) {
            paint2= new Paint();
            // 去锯齿
        ...
        float yinTemp = 10;
        path.moveTo(x,y - vLenght-yinTemp);
        path.lineTo(x + vLenght * (float)Math.sqrt(3)+yinTemp / 2,y - vLenght / 2 - yinTemp);
        path.lineTo(x + vLenght * (float)Math.sqrt(3)+yinTemp / 2,y + vLenght / 2 + yinTemp);
        path.lineTo(x, y + vLenght+yinTemp);
        path.lineTo(x - vLenght * (float)Math.sqrt(3)-yinTemp / 2,y + vLenght / 2+yinTemp);
        path.lineTo(x - vLenght * (float)Math.sqrt(3)-yinTemp / 2,y - vLenght / 2-yinTemp);
        path.close();
